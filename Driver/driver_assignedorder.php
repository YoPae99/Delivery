...
function fetchAssignedOrders($driverId) {
    global $pdo;
    // Fetch assigned orders that are in progress or have no status
    $stmt = $pdo->prepare("
        SELECT o.OrderId, u.Name, u.Email, o.Address, o.OrderDate, o.Status
        FROM orders o 
        JOIN user u ON o.ClientId = u.ID 
        WHERE o.DriverId = :driverId AND (o.Status IS NULL OR o.Status != 'delivered')
    ");
...
<main>
    ...
    <div id="status-update-container" class="border rounded p-4">
        <div>
            <h3 style="font-size: 30px">Assigned Orders</h3>
            <hr>
        </div>
